...
public class VacinaDatabaseMock {
    ...
    public boolean addNewVacina(String nome, String descricao, String lote, String fabricante){
        Vacina vacina = new Vacina(newId(), nome, descricao, lote, fabricante);

        vacinas.add(vacina);

        return vacinas.contains(vacina);
    }

    private int newId(){
        return vacinas.get(vacinas.size() - 1).getId() + 1;
    }
}
